...
def load_and_plot(*args):
    ...
    if (os.name != 'nt'):                                # Linux/Unix
	    ...
	    cmd_feedback = check_output(cmd, shell=True)
	    cmd = './make_file_list_unix ' + raw_dir + ' data* > ' + MCS_file_list
	    cmd_feedback = check_output(cmd, shell=True)
    else:                                                # Windows
        cmd = 'type nul > ' + MCS_file_list
        cmd_feedback = check_output(cmd, shell=True)
        cmd = 'del ' + MCS_file_list
        cmd_feedback = check_output(cmd, shell=True)
        cmd = 'dir ' + raw_dir + 'data* /B/S/OD > ' + MCS_file_list 
        cmd_feedback = check_output(cmd, shell=True)
    ...
    if (file_ctrl.sel_file_list == new_sfl) is False:
        CAMAL_data_obj.ingested = False
    print("-----> Ingested = ",CAMAL_data_obj.ingested)        
    file_ctrl.sel_file_list = new_sfl        
        
    # Read in entire dataset ---

    if CAMAL_data_obj.ingested is False:
        ingest = ingest_entire_dataset(MCS_file_list,CAMAL_data_obj,file_ctrl)
        # If the above line return no usable data, "ingest" will be false
        if not ingest: return None
        ...
        CAMAL_data_obj.set_time_axis_array()
        # Determine the look angles within data & set their mask array
        CAMAL_data_obj.determine_look_angles()
        ...
        CAMAL_data_obj.nprofs0 = CAMAL_data_obj.nprofs
        for angle in CAMAL_data_obj.hist_bin_centers:
    ...
    CAMAL_data_obj.set_bin_alt_array(pointing_dir,yax_type,ang_list.current()-1)
    nc = CAMAL_data_obj.std_params[0]
    nb = CAMAL_data_obj.std_params[1]
    nshots = CAMAL_data_obj.std_params[2]
    vrT = CAMAL_data_obj.std_params[3]
    vrZ = CAMAL_data_obj.std_params[4]
    nr = CAMAL_data_obj.nprofs
    ...
    for i in range(0,len(selected_chan)):
        if not CAMAL_data_obj.avgd_mask[selected_chan[i]-1]:
            if nhori not in range(1,200):
                print('Set a more reasonable averaging # in initialization file.')
                return None
            elif (nhori > 1):
                samp_chan = CAMAL_data_obj.data['counts'][:,selected_chan[i]-1,:]
                samp_chan = average_lidar_data(samp_chan,nhori,0)
                new_shape = samp_chan.shape
                CAMAL_data_obj.nprofs = new_shape[0]
                CAMAL_data_obj.update_avgd_mask(selected_chan[i])
                CAMAL_data_obj.data['counts'][0:new_shape[0],selected_chan[i]-1,:] = samp_chan
    ...
    samp_chan = CAMAL_data_obj.data['counts'][0:CAMAL_data_obj.nprofs0,
                                    selected_chan[0]-1:selected_chan[len(selected_chan)-1],
                                    :].sum(1)
    CAMAL_data_obj.nprofs = CAMAL_data_obj.nprofs0 # RESET nprofs here!!!                                  
    ...
    if curtain_type > 1:
        ...
        EMs = convert_raw_energy_monitor_values(CAMAL_data_obj.data['meta']['EM'],nwl)
        EMsubmit = EMs[:,wl]
        ...
        samp_chan = correct_raw_counts(samp_chan,EMsubmit,np.arange(0,nb*vrZ,vrZ),
                                       CAMAL_data_obj.nprofs,nb,bg_st_bin,bg_ed_bin,
                                       opt)
    # Store samp_chan for possible profile plotting (samp_chan_mask set in here!)                                      
    CAMAL_data_obj.samp_chan = samp_chan   
    ...
    CAMAL_data_obj.apply_look_angle_mask(ang_list.current()-1,ang_mask_opt)                                                                       
    # Manipulate samp_chan to get correct image orientation
    samp_chan = CAMAL_data_obj.samp_chan
    ...
    title = 'CAMAL Curtain Plot - channel ' + "".join(chan_sel.get().split()) + ' - ' + wl_str[wl] + \
            ' nm ' + lab_tag
    if xax_type == 'time':
        nprofs0 = CAMAL_data_obj.data['meta']['lon'].shape
        xax_lims = [ mdates.date2num(CAMAL_data_obj.time_ax_arr[CAMAL_data_obj.samp_chan_map[0]*nhori]), 
                   mdates.date2num(CAMAL_data_obj.time_ax_arr[ 
                   CAMAL_data_obj.samp_chan_map[(CAMAL_data_obj.nprofs)-1]*nhori ]) ]
        xlabel = 'UTC'                   
    else:
        xax_lims = [ 0, CAMAL_data_obj.nprofs ]
        xlabel = 'Record number'
    CPlot, im_width, im_Bbox = make_curtain_plot(samp_chan,nb,vrZ,
                                                 CAMAL_data_obj.z,canvas_ctrl, color_bar_min,
    ...
    canvas_ctrl.pix2prof = CAMAL_data_obj.nprofs/im_width
...
def canvas_Lclick(event):
    ...
    if canvas_ctrl.var_window_open is True:
        ...
        if event.x > canvas_ctrl.img_bounds[0] and event.x < canvas_ctrl.img_bounds[2]:
            ...
            #print('Prof # ', prof_num, ' of ', CAMAL_data_obj.nprofs)
            ...
            nx0 = CAMAL_data_obj.data['meta'].shape
            # The "prof_num*nhori" will account for any averaging of data (any nhori >=1)
            prof_transl = CAMAL_data_obj.samp_chan_map[prof_num]*nhori
            prof_plot_title = 'UTC: ' + DT.datetime.strftime(CAMAL_data_obj.time_ax_arr[prof_transl],datetimeformat) + \
                              '\nlat: ' + str(CAMAL_data_obj.data['meta']['lat'][prof_transl]/1e2)[0:6] + \
                              ', lon: ' + str(CAMAL_data_obj.data['meta']['lon'][prof_transl]/1e2)[0:6]
            plt.title(prof_plot_title)
            canvas_ctrl.update_any_canvas_ctrl_plot(CAMAL_data_obj.samp_chan[prof_num,:],
                                         CAMAL_data_obj.z, fig2)
...
def plot_Emon():
    ...
    if CAMAL_data_obj.ingested is False:
        ...
        nprofs0 = CAMAL_data_obj.data['meta'].shape
        xax_type = xax_drop.get()      # get user-selected axis type
        if xax_type == "time":
            xvals = CAMAL_data_obj.time_ax_arr
        ...
        plt.title('CAMAL energy monitors')
        lines2D_obj = EMp1
...
img_bounds = [0,0,0,0]
CAMAL_data_obj = lidar_data_obj(None, None, False, None, None, False, None,
...
#
# For CAMAL:
# 1064 is channels 1 and 2
# 532 is channels 3 and 4
# 355 is channel 5
