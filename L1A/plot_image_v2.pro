PRO plot_image_v2,data,x,y,absmax,posvec,xtotsize,ytotsize,tit1,tit2,xtit,ytit,cbtit,filename,num_x_axes,x2,x3,ct
...
;
;In v2, ct is an array that can be loaded by the IDL TVLCT procedure.   [3/30/2017]
...
;ct         -> A 3 x N dimension array containing the RGB levels for the color table
;
;OUTPUTS:
;(none)
...
;/*** FORM IMAGE ***\
ndivisions = n_elements( ct[0,*] )
RED = congrid( reform(ct[0,*]) , 256 )
GREEN = congrid( reform(ct[1,*]) , 256 )
BLUE = congrid( reform(ct[2,*]) , 256 )
...
colorbar2,maxrange=absmax,position=cbpos,division=ndivisions,charsize=cbchar,/vertical,/right,color=0
