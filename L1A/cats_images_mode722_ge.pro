;;;;;;;;;;;;;;;;; cats_images_mode72_ge.pro ;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
;       I INTEND FOR THIS TO BE A USEFUL DESCRIPTION (10/20/16)
;
;	Reads in CATS L1B Data for Mode 7.2 and creates all neccessary 
...
;	Version:
;
;		Version ?. Last edited by Patrick Selmer Oct. 20, 2016
;					
;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

PRO cats_images_mode722_ge,config_path
...
dum=''
openr,clun,config_path+'cats_images_mode722_ge.config',/get_lun
...
readf,clun,dum
strings = strsplit(dum,' ',/extract)
file1_name=strings[2]
readf,clun,dum
strings = strsplit(dum,' ',/extract)
file2_name=strings[2]
readf,clun,dum
strings = strsplit(dum,' ',/extract)
...
if (file_path ne 'x') then begin
    infile1 = file_search(file_path , file1_name)
    infile1 = infile1[0]
    infile2 = file_search(file_path , file2_name)
    infile2 = infile2[0]
endif else begin
    infile1 = dialog_pickfile(path=dialog_path)
    infile2 = dialog_pickfile(path=dialog_path)
endelse


gname = strmid(infile1,fnlen,57,/reverse_offset) ;granule name w/no path
type='FFOV'
...
;Read in L1B Data from 1st infile
CATS_L1B_HDF5_v2_04_READER, infile1			;Sub-Routine to read in L1B data
;This next chunk of code is responsible for making single giant arrays to hold data from both grans
CATS_Fore_FOV_Latitude1=CATS_Fore_FOV_Latitude
CATS_Fore_FOV_Longitude1=CATS_Fore_FOV_Longitude
Total_Attenuated_Backscatter532_Fore_FOV1=Total_Attenuated_Backscatter532_Fore_FOV
Total_Attenuated_Backscatter1064_Fore_FOV1=Total_Attenuated_Backscatter1064_Fore_FOV
Perpendicular_Attenuated_Backscatter1064_Fore_FOV1=Perpendicular_Attenuated_Backscatter1064_Fore_FOV
Profile_UTC_Time1=Profile_UTC_Time
numrecs1=Number_Profiles
;Read in L1B Data from 2nd infile
CATS_L1B_HDF5_v2_04_READER, infile2			;Sub-Routine to read in L1B data
numrecs2=Number_Profiles
numrecs=numrecs2+numrecs1
CATS_Fore_FOV_Latitude=[CATS_Fore_FOV_Latitude1,CATS_Fore_FOV_Latitude]
CATS_Fore_FOV_Longitude=[CATS_Fore_FOV_Longitude1,CATS_Fore_FOV_Longitude]
Profile_UTC_Time=[Profile_UTC_Time1,Profile_UTC_Time]
Total_Attenuated_Backscatter532_Fore_FOVnew=fltarr(number_bins,numrecs2+numrecs1)
Total_Attenuated_Backscatter532_Fore_FOVnew[*,0:numrecs1-1]=Total_Attenuated_Backscatter532_Fore_FOV1
Total_Attenuated_Backscatter532_Fore_FOVnew[*,numrecs1:numrecs-1]=Total_Attenuated_Backscatter532_Fore_FOV
Total_Attenuated_Backscatter532_Fore_FOV=Total_Attenuated_Backscatter532_Fore_FOVnew
Total_Attenuated_Backscatter1064_Fore_FOVnew=fltarr(number_bins,numrecs2+numrecs1)
Total_Attenuated_Backscatter1064_Fore_FOVnew[*,0:numrecs1-1]=Total_Attenuated_Backscatter1064_Fore_FOV1
Total_Attenuated_Backscatter1064_Fore_FOVnew[*,numrecs1:numrecs-1]=Total_Attenuated_Backscatter1064_Fore_FOV
Total_Attenuated_Backscatter1064_Fore_FOV=Total_Attenuated_Backscatter1064_Fore_FOVnew
Perpendicular_Attenuated_Backscatter1064_Fore_FOVnew=fltarr(number_bins,numrecs2+numrecs1)
Perpendicular_Attenuated_Backscatter1064_Fore_FOVnew[*,0:numrecs1-1]=Perpendicular_Attenuated_Backscatter1064_Fore_FOV1
Perpendicular_Attenuated_Backscatter1064_Fore_FOVnew[*,numrecs1:numrecs-1]=Perpendicular_Attenuated_Backscatter1064_Fore_FOV
Perpendicular_Attenuated_Backscatter1064_Fore_FOV=Perpendicular_Attenuated_Backscatter1064_Fore_FOVnew
...
lat = CATS_Fore_FOV_Latitude[srec:erec] ;ISS_Latitude[srec:erec]
lon = CATS_Fore_FOV_Longitude[srec:erec];ISS_Longitude[srec:erec]
timestrarr = timestrarr[srec:erec]

nrecs=erec-srec+1
nbin=nz1 ;Number_Bins
cats_atb_532_ffov = transpose(Total_Attenuated_Backscatter532_Fore_FOV[zlocs,srec:erec])
cats_atb_1064_ffov = transpose(Total_Attenuated_Backscatter1064_Fore_FOV[zlocs,srec:erec])
cats_atb_1064perp_ffov = transpose(Perpendicular_Attenuated_Backscatter1064_Fore_FOV[zlocs,srec:erec])
cats_atb_1064par_ffov = transpose(Total_Attenuated_Backscatter1064_Fore_FOV[zlocs,srec:erec]-Perpendicular_Attenuated_Backscatter1064_Fore_FOV[zlocs,srec:erec])

avrecs=nrecs/rec_sum[0]
cats_bks_532_ffov=dblarr(avrecs,nbin)
cats_bks_1064_ffov=dblarr(avrecs,nbin)
cats_depol_532_ffov=dblarr(avrecs,nbin)
cats_depol_1064_ffov=dblarr(avrecs,nbin)
data = dblarr(avrecs,nbin) ;array to pass to plot_image program
temp = dblarr(nz1)
cratio = dblarr(avrecs,nbin) 
...
for r=0,avrecs-1 do begin
   ...
   if r eq avrecs-1 then begin
	...
	cats_bks_532_ffov(r,b)=mean(cats_atb_532_ffov(begrec:nrecs-1,b))
	cats_bks_1064_ffov(r,b)=mean(cats_atb_1064_ffov(begrec:nrecs-1,b))
	cats_bks_1064perp_ffov=mean(cats_atb_1064perp_ffov(begrec:nrecs-1,b))
	cats_bks_1064par_ffov=mean(cats_atb_1064par_ffov(begrec:nrecs-1,b))
	if (cats_bks_1064_ffov(r,b) ge depolbscutoff) then begin	 	 
	  cats_depol_1064_ffov(r,b)=cats_bks_1064perp_ffov / cats_bks_1064par_ffov
	  cratio(r,b) = cats_bks_1064_ffov(r,b) / cats_bks_532_ffov(r,b)
	endif 
      endfor
    endif else begin
    ...
    for b=0,nbin-1 do begin
      ...
      cats_bks_532_ffov(r,b)=mean(cats_atb_532_ffov(begrec:endrec,b))
      cats_bks_1064_ffov(r,b)=mean(cats_atb_1064_ffov(begrec:endrec,b))
      cats_bks_1064perp_ffov=mean(cats_atb_1064perp_ffov(begrec:endrec,b))
      cats_bks_1064par_ffov=mean(cats_atb_1064par_ffov(begrec:endrec,b))
      if cats_bks_1064_ffov(r,b) ge depolbscutoff then begin
         cats_depol_1064_ffov(r,b)=(cats_bks_1064perp_ffov)/ cats_bks_1064par_ffov
         cratio(r,b) = cats_bks_1064_ffov(r,b) / cats_bks_532_ffov(r,b)	 
      endif
  ...
  CASE wavelength[0] OF
       1064: temp[0l:nz1-1l] = cats_bks_1064_ffov[r,*] ;!!!!!! SET CURTAINPLOT VARIABLE HERE <<<<<<<<<<<<<<<<<<
        532: temp[0l:nz1-1l] = cats_bks_532_ffov[r,*]
          0: temp[0l:nz1-1l] = cats_depol_1064_ffov[r,*]
  ENDCASE
  ;temp = reverse(temp)
  data[r,*] = temp
endfor  
...
ENDCASE
CASE wavelength[0] OF
     1064: data_label = 'bare_1064_ffov' 
      532: data_label = 'bare_532_ffov'
        0: data_label = 'bare_1064depol_ffov'
ENDCASE
ytit = 'Altitude (km)'
