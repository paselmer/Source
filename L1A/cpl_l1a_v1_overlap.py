...
raob_data = read_in_esrl_raob(raob_file)
# Remove records with invalid pres, alt, temp, or dewp
mask = ((raob_data['pres'] != 99999) & (raob_data['alt'] != 99999) &
       (raob_data['temp'] != 99999) & (raob_data['dewp'] != 99999)) 
raob_data = raob_data[mask]

# Convert raob data into appropriate variables to pass to vmol_interp

ht_r = raob_data['alt'] / 1000.0 # m to km
pres_r = raob_data['pres'] / 10.0 # pa to hpa
tmp_r = raob_data['temp']/10.0 + 273.16 #dC to K
dewp_r = raob_data['dewp']/10.0 + 273.16
esat = 0.6112 * np.exp((17.67*(tmp_r-273.16))/(tmp_r-29.66))
qsat1 = (0.622 * esat) / (pres_r/10.0) # based on temperature
esat = 0.6112 * np.exp((17.67*(dewp_r-273.16))/(dewp_r-29.66))
qsat2 = (0.622 * esat) / (pres_r/10.0) # based on dewpoint temperature
rel_humidity = ( qsat2 / qsat1 ) *100.0
num_levels = raob_data.shape[0]
sbin = 24.0 #km
sratm = 8.0*np.pi/3.0 # molecular lidar ratio

#inputs = [ht_r, pres_r, tmp_r, rel_humidity, num_levels, tempfile, stop_raob_ht, nwl, sbin, vrZ/1e3]
#outputs = vmol_interp(inputs)
...
for f in range(0,nCLS_files):
    ...
    while i1f < nr_1file:
        ...
        # Compute the attenuated Rayleigh backscatter at each of the exact raw bin altitudes
        #
        # First, choose bin altitude that's closest and above the minimum raob ht
        min_bin_alt_abv_first_raob_ht = bin_alts[bin_alts >= ht_r[0]*1e3].min()
        min_bin_num_abv_first_raob_ht = bin_alts[bin_alts >= ht_r[0]*1e3].argmin()
        sbin = min_bin_alt_abv_first_raob_ht
        delz = min_bin_alt_abv_first_raob_ht - ht_r[0]*1e3
        if delz <= 0: pdb.set_trace() # should not occur. if does, rethink.
        inputs = [ht_r, pres_r, tmp_r, rel_humidity, num_levels, tempfile, stop_raob_ht, nwl, sbin/1e3, delz/1e3]        
        Bray, nbins_interp, ht_x, pres_x, tmp_x, rh_x = vmol_interp(inputs) # compute molecular backscatter
        ht_u = np.copy(ht_r) # u stands for "use"
        ...
        rh_u[0] = rh_x[-2]
        sbin = bin_alts.max()
        vrZ_computed = (bin_alts[:-1] - bin_alts[1:]).mean()
        inputs = [ht_u, pres_u, tmp_u, rh_u, num_levels, tempfile, stop_raob_ht, nwl, sbin/1e3, vrZ_computed/1e3]        
        outputs = vmol_interp(inputs) # compute molecular backscatter
        Bray, nbins_interp, Bray_alt = outputs[:3]
        test_dz = Bray_alt[0]-bin_alts[0]/1e3
        if test_dz > 0.6*(vrZ_computed/1e3):  # first elem of Bray_alt should be bin_alts.max() + vrZ_computed
            Bray = Bray[1:,:]      
            Bray_alt = Bray_alt[1:]
        # Compute slantwise Molecular Transmission at all raw range bins
        odm = np.zeros(nwl,dtype=np.float64) - 0.5 * np.log(1.0)
